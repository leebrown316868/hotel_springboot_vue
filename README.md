...
| **MySQL** | 8.0 | 关系型数据库 |
| **Alipay SDK** | 4.39.218 | 支付宝沙箱支付 |
| **Spring Dotenv** | 4.0.0 | 环境变量管理（.env 文件） |
...
本系统采用 **MySQL 8.0** 作为数据库引擎，主要考虑因素：

- **高性能关系型数据库**：成熟的 ACID 事务支持，满足酒店业务数据一致性要求
- **丰富的生态**：Spring Data JPA + Hibernate 原生支持，开发效率高
- **适合中小型应用**：满足中小型酒店的业务数据量需求，同时具备良好的扩展性
- **外键约束**：支持完整的外键约束，保障数据完整性
...
| preferences_enabled | TINYINT(1) | DEFAULT 1 | 偏好设置开关 |
| role | VARCHAR(50) | NOT NULL | 角色（ADMIN/STAFF/CUSTOMER） |
...
| 字段名 | 类型 | 约束 | 说明 |
|--------|------|------|------|
| id | INT | PRIMARY KEY, AUTO_INCREMENT | 客房ID |
| number | VARCHAR(50) | NOT NULL, UNIQUE | 房间号 |
| floor | VARCHAR(10) | NOT NULL | 楼层 |
| type | VARCHAR(50) | NOT NULL | 房型（STANDARD/DELUXE/SUITE等） |
| status | VARCHAR(50) | NOT NULL | 房态（AVAILABLE/OCCUPIED/MAINTENANCE/CLEANED） |
...
| total_bookings | INT | DEFAULT 0 | 总预订次数 |
...
| booking_id | INT | FK → bookings(id), UNIQUE | 关联订单（一对一） |
| user_id | INT | FK → users(id) | 评价用户 |
| guest_id | INT | FK → guests(id) | 关联客户 |
...
| 方法 | 路径 | 说明 | 权限 |
|------|------|------|------|
| POST | `/api/payment/alipay/create` | 创建支付宝支付订单 | 公开 |
| POST | `/api/payment/alipay/notify` | 支付宝异步回调通知 | 公开 |
| GET | `/api/payment/status/{bookingNumber}` | 查询支付状态 | 公开 |
...
# 编辑 .env 文件，填入实际配置
# - JWT_SECRET: JWT 密钥
# - ZHIHUI_APP_ID: 支付宝沙箱 App ID
# - ZHIHUI_PRIVATE_KEY: 支付宝沙箱私钥
# - ZHIHUI_PUBLIC_KEY: 支付宝公钥
...
mysql -u root -p -e "CREATE DATABASE IF NOT EXISTS hotel_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"

# 2. 确保已安装 JDK 17 和 Maven
java -version
mvn -version

# 3. 在项目根目录下启动 Spring Boot
mvn spring-boot:run
```

> 后端服务将在 `http://localhost:8080` 启动，首次启动时会自动执行建表脚本和初始数据填充。
...
- [x] ~~**数据库升级至 MySQL**：支持更大规模数据~~ ✅ 已完成
- [x] ~~**微信/支付宝在线支付集成**：接入第三方支付接口实现真实支付~~ ✅ 已完成（支付宝沙箱）
